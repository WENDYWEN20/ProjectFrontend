...
export default function ProjectDetails({ projects, setProjects }) {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      ...
      setUpdateFormData(data.project); //form data for updating current project
    };
    fetchProject();
  }, [params.id]);
  ...
  if (!project) {
    return (
      <div>
        <h2>ProjectDetailPage not found</h2>
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      console.log(`check updatFormData : ${updateFormData}`) 
      //makes PUT request to update projects
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/projects/${params.id}`,
        {
    ...
    <main>
      ...
      <div>
        <button onClick={handleDelete}>
          Deleteeeeeeee <MdOutlineDelete size={24} />
        </button>
      </div>
